...
**The eval:coach law:** re-run `CLAUDE_API_KEY=... npm run eval:coach` LIVE after ANY prompt or model change to `api/coach-read.js`, and judge the 9 reads against the F5 bar before deploying. The console states each persona's TRUE state (passed / N checks failed / API call failed) and the run **exits non-zero** if any hard check fails — a tick used to mean only "control reached this line" (July 29, 2026). Last live run: **July 29, 2026 (second run, against the current prompt)** — **substance passed**: 9/9 parsed, voice scan flagged zero, the confident-error headline rule passed, the freezer timeout rule passed, and the fabricated-statistic failure from the first run is **fixed** (the model no longer tallies a list — counts arrive pre-grouped per villain). **Prompt v2 VALIDATED (July 29, 2026, 4th live run):** trajectory tier, trigger-action watchFor, caps 12/24/20 — every substance check green (false-improvement guard 5/5, trajectory 4/4, tier-1, freezer, voice). Word caps carry a **±2-word tolerance** (soft ⚠, reported; +3 is a hard ✗) — a founder-delegated policy set AFTER substance converged, pre-committed for all future runs (`CAP_TOLERANCE` in eval-coach.mjs); widening it to green a run is the same sin as moving a cap. Substance checks have no tolerance. `--dry` writes a SEPARATE file (`coach-eval-dry-prompts.md`) and can never overwrite the live artifact; both state their run mode and timestamp on line 1, and the harness reports how much it measured, not just ✓/✗ (July 29, 2026 — a dry run destroyed a live artifact and a whole scoring round was run against placeholders).
