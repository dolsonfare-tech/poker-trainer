...
- **`App.jsx` currently owns auth/session orchestration** (auth listener, guest flow, session run state, coach-read pipeline). The "App is routing only" statement is **aspirational** — it becomes true only after TARGET_ARCHITECTURE Wave 3 lands (hooks + contexts). Target end-state and refactor ordering → `docs/architecture/TARGET_ARCHITECTURE.md`.
