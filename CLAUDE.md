...
**The eval:coach law:** re-run `CLAUDE_API_KEY=... npm run eval:coach` LIVE after ANY prompt or model change to `api/coach-read.js`, and judge the 9 reads against the F5 bar before deploying. The console states each persona's TRUE state (passed / N checks failed / API call failed) and the run **exits non-zero** if any hard check fails — a tick used to mean only "control reached this line" (July 29, 2026). Last live run: **July 29, 2026** — 9/9 reads returned and parsed, the confident-error headline rule passed (1 applicable, 1 passed), the voice scan flagged zero, and three reads breached a word cap (headline 13w vs 12, an evidence item 24w vs 20, watchFor 19w vs 18). **The CURRENT prompt has NOT passed a live eval:** both the prompt and the payload changed after that run (citations withhold the skill tag on sub-bar skills; the spot now carries the board), so a further live run is owed before deploy. `--dry` writes a SEPARATE file (`coach-eval-dry-prompts.md`) and can never overwrite the live artifact; both state their run mode and timestamp on line 1, and the harness reports how much it measured, not just ✓/✗ (July 29, 2026 — a dry run destroyed a live artifact and a whole scoring round was run against placeholders).
