...
|---|---|---|---|
| 1 | `npm run check:invariants` | after EVERY code change | 36 architecture rules (single-file ownership incl. schema/persistence/dates/event-name owners, secrets, RLS, no-async-onAuthStateChange, asset budgets, dead-layout guard, CI-status watchdog, triage-doc event paths, root-doc allowlist, CLAUDE.md line budget, component + hook line budgets, test co-location, frozen-clock, server-ESM-resolvable, coach tenant scope, coach-eval contract + dry-run artifact safety, coach-module interop, suit-glyph text presentation) |
| 2 | `CI=true npm test` | after every code change | jest suite (unit + integration + source pins) |
| 2b | **`npm run gates`** — the whole set as one command; **prefer it over running 1 and 2 individually** | after every code change | mirrors every CI step except e2e: invariants → both content audits (incl. the scenario-index staleness check) → `smoke:coach -- --selftest` (offline; the live probe is gate 8) → jest → `simulate:schemas` → `playtest:personas` → `CI=true npm run build` → `check:bundle`. Two red-CI incidents on July 27, 2026 came from running a SUBSET locally: a lint-only `exhaustive-deps` error (`CI=true` promotes warnings to errors — jest and e2e don't lint), then a deleted module still imported by `scripts/` (the harnesses import from `src/` but no `src/` test exercises them). If a gate isn't in this script, it will eventually be skipped |
...
| 6b | `npm run e2e:auth` | `SignIn.jsx`, `useGuest.js`, the guest gate | the sign-in screen — guest-first hierarchy, reveal toggle, magic-link success/error. **Requires `npm run e2e:build:auth` first** (DUMMY Supabase env): auth mode is decided at BUILD time, so SignIn is unreachable from lane 1's build and this lane is unreachable from lane 1's specs. Each lane rejects the other's build (invariants rule 37) |
...
| 8 | `npm run smoke:coach` | after EVERY deploy touching `api/coach-read.js` or anything it imports from `src/` | the DEPLOYED lambda — the only check that leaves the local machine. Every other gate loads the real ESM; the lambda loads Vercel's CommonJS transpilation of the same files, which is exactly how every production read broke on July 29, 2026 with all gates green. Tier 1 is free and needs no token (`GET → 405` = it boots; `POST → 401` = all three server env vars are set — a `400` there means the Supabase vars are gone and auth, the daily cap AND the tenant scope were all skipped). Tier 2 needs `COACH_SMOKE_TOKEN`; an account with **no sessions** proves the `api/ → src/` import for free, since the empty-log guard sits after `loadModules()` |

**The eval:coach law:** re-run `CLAUDE_API_KEY=... npm run eval:coach` LIVE after ANY prompt or model change to `api/coach-read.js`, and judge the 9 reads against the F5 bar before deploying. The console states each persona's TRUE state (passed / N checks failed / API call failed) and the run **exits non-zero** if any hard check fails — a tick used to mean only "control reached this line" (July 29, 2026). Last live run: **August 2, 2026 — prompt v3 VALIDATED on its 4th live run: 9/9 personas clean, exit 0** (advice voice, two fields, zero numerals; substance green with no tolerance — tier-1 1/1, trajectory clause 4/4, false-improvement 5/5, freezer, voice/fingerprint/em-dash scans silent). v3's caps are **48 total / 22 headline / 30 watchFor**, re-registered OPENLY mid-loop after runs 1–2 showed the founder's chosen register measuring 42–50 words (basis + why-this-is-not-the-tolerance-sin: the v3 spec's re-registration section). Runs 1–3 each bought a real fix, twice in the HARNESS not the prompt (the "spend the difference" instruction backfire; the IMPROVEMENT_VOCAB noun lookahead — "the stronger play" is an action comparative, not a trajectory claim). Word caps carry a **±2-word tolerance** (soft ⚠, reported; +3 is a hard ✗) — a founder-delegated policy set AFTER substance converged, pre-committed for all future runs (`CAP_TOLERANCE` in eval-coach.mjs); widening it to green a run is the same sin as moving a cap. Substance checks have no tolerance. `--dry` writes a SEPARATE file (`coach-eval-dry-prompts.md`) and can never overwrite the live artifact; both state their run mode and timestamp on line 1, and the harness reports how much it measured, not just ✓/✗ (July 29, 2026 — a dry run destroyed a live artifact and a whole scoring round was run against placeholders). **A green eval is not a working deploy** — it proves the prompt, not the lambda. Finish with gate 8, and never accept "watch PostHog `coach_read_failed`" as verification: that signal only fires when a real user triggers a read, so with no traffic its healthy state and its dead state are the same zero (that is how the v3 read sat unverified from August 2 to August 30, 2026).
...
│   │   ├── scenario-index.js  ← GENERATED compact direction index (npm run gen:scenario-index).
│   │   │                        Lets schema.js diagnose without the 438 KB library (CA-014).
│   │   ├── villains.js        ← VILLAIN_LABELS — extracted so 8 strings don't pull the pool in.
