...
**Founder ideas raised July 5 — to consider before launch (captured, no decision yet; neither is in build scope until founders decide):**
1. **Coin economy for sessions** — 5 free coins per day, 1 coin per session, $2 to refill the 5 coins; the daily 5 auto-refresh 24 hours after the first coin is used. Would be the first paid mechanic (before the Pro tier) and the first cap on daily play. ⚠️ Tensions to resolve if adopted: the standing monetization decision is "free + ads at launch" with unlimited daily play, and the July 5 copy pass deliberately removed all daily-limit claims ("Sign in and play for free.") — copy and positioning would need a coordinated pass. Also needs: payment rails (none exist yet — Stripe or similar), server-side coin ledger (Supabase table + RLS, enforced in the session-start path, not client-only), and a decision on how it coexists with ads.
2. **"Disagree?" box on the feedback screen** — after the answer is graded, a small box: "Disagree? Let us know if we have this wrong" with a few fixed responses (no free text), so common objections aggregate per scenario and the most-disputed scenarios/gradings surface for review. Complements the SME grading review and the July 5 audit (which proved gradings do have errors — sc_028 etc.). Cheap to build: reuse the beta-feedback pattern (Supabase insert-only table or extend `feedback` with `scenario_id` + fixed-choice category, PostHog event with scenario id). Design care: don't let it undermine confidence in the coaching on the decision screen — keep it quiet, post-feedback only. Fixed-response options TBD by founders (e.g. "the graded answer is wrong" / "my answer deserves partial credit" / "the explanation doesn't match the board" / "sizing seems off").
