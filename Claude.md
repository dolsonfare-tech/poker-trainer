...
- **Sessions are dealt by the session builder** (`src/utils/spacedrep.js`, July 2026) — NOT pure random: unseen first, weak-skill weighting, one resurfaced miss max (labeled with the replay chip — never resurface silently), and **max 2 preflop-street hands per 5** (founder, July 8: weak preflop/position skills were making sessions ~50% boardless preflop spots; the cap is soft — yields when the pool leaves no choice; postflop streets uncapped). Per-hand `scenarioId` in the `sessions.hands` payload is load-bearing (history rebuilds from it); don't remove it.
