...
12. **Founder polish queue — July 29 2026 (evening).** Six items handed over as one brief; four shipped, one was already done, one is parked on new art.
    - ✅ **Timer kept running behind the guide modal.** The worst first-impression bug in the repo: tapping ⓘ (header button or the villain read on the felt) opened a modal over a live countdown, so consulting the help could time the hand out. `paused={decided}` was the only input that ever froze the ring. **Ratchets:** `CanvasLayout.test.js` pins both edges (freeze AND resume — a pause that never resumes is the same bug in a different hat) plus "an open guide cannot time the player out"; `App.integration.test.js` pins the WIRING, which is the half that was actually broken — the prop can be dropped at the call site with every CanvasLayout test still green. That test reproduced the bug at 57→27 before the fix.
    - ✅ **Bare ♠ glyphs rendered as colour emoji on Apple platforms.** The July 27 fix pinned exactly one site (`DifficultySelector`) and left `SignIn`'s guest note and the Dashboard guest avatar to be re-reported on July 28 — pinning one site never generalises. Found 13 unpinned sites, not 2: the other 11 are `VillainGuide`'s glossary prose, same bug, never reported because nobody reads the glossary on an iPhone. **Ratchet (the one the first fix didn't leave):** invariant 36 — every suit glyph in component JSX carries U+FE0E, with a `raw-suit-ok` opt-out for comparisons against unpinned data (`TableCanvas`'s `isRed`), plus a generic CSS half asserting App.css never names an emoji font family. Both halves verified by negative control.
