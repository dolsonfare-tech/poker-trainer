...
3. ✅ **DONE 2026-07-28** — CA-022 route-level code splitting. `React.lazy()` + `<Suspense>` for `TableReads` and `VillainGuide`. `SessionSummary` was considered and REJECTED: the doc said "optionally", and the answer is no — every finisher needs it at the results reveal, so a ~4 KB gzip saving buys a stutter at the one moment the player is waiting for a payoff. Split by who reaches a surface, not by how big it is.
