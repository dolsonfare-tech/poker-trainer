...
const SCENARIOS = [
  ...
  mkScenario({
    ...
    positions: mkPositions({
      0: { label: 'UTG',      action: 'Active', state: 'active' },
      2: { label: 'CO',       action: 'Active', state: 'active' },
      5: { label: 'BB (You)', action: '???',    state: 'hero'   },
    }),
    hand: mkHand(['7','♥'], ['7','♣']),
  ...
  mkScenario({
    ...
    choices: [
      ...
      {
        val: 'check', label: 'Check behind', icon: '📞', cls: 'call',
  ...
  mkScenario({
    ...
    choices: [
      {
        val: 'fold', label: 'Donk-bet $12', icon: '🃏', cls: 'fold',
      ...
      {
        val: 'call', label: 'Check — let him bet', icon: '📞', cls: 'call',
      ...
      {
        val: 'raise', label: 'Check-raise if he bets', icon: '⚡', cls: 'raise',
  ...
  mkScenario({
    ...
    pot: '$30',
    toCall: null,
    body: "BTN aggressive regular raised. You called BB with K♠7♠. Flop K♥8♦3♠ — he c-bets, you call. Turn 9♣ — he checks. Aggressive regulars who check the turn after c-betting usually have a capped range.",
  ...
  mkScenario({
    ...
    tableContext: null,
    positions: mkPositions({
      2: { label: 'CO (You)', action: 'Called $6', state: 'hero'   },
      5: { label: 'BB (M)',   action: 'Raised $6', state: 'active' },
    }),
    hand: mkHand(['J','♥'], ['J','♣']),
    board: ['J♦', '6♠', '2♥'],
    pot: '$14',
    ...
    choices: [
      {
        ...
        fb: "Check back. A calling station who called flop and turn on KT39 has you beat at high frequency — a ten, a nine, TJ (straight), or two pair. Your river bet extracts value from fewer hands than it loses to.",
      },
      {
...
correct: 'bet_medium',
  ...
  mkScenario({
    ...
    choices: [
      {
        val: 'fold', label: 'Fold', icon: '🃏', cls: 'fold',
        grade: 'incorrect', title: "Full House Doesn't Fold", emoji: '❌',
        fb: "Folding a full house to an overbet is impossible. His polarized bet range includes bluffs — call or re-raise, never fold.",
      },
      {
        val: 'call', label: 'Call $70', icon: '📞', cls: 'call',
        grade: 'partial', title: 'Calling is Fine But Raise for Value', emoji: '⚠️',
        fb: "Calling is fine but leaves value behind. You have a full house against a loose player who overbets polarized — re-raise and get called by the rare hand he's doing this with.",
      },
      {
        val: 'raise', label: 'Raise to $160', icon: '⚡', cls: 'raise',
        grade: 'correct', title: "Re-Raise — He's Polarized and Calls", emoji: '✅',
        fb: "Re-raise to $160. You have a full house — the loose recreational's overbet is either the nuts or a bluff. Against a player who overbets both, re-raising gets called by his nuts and wins his bluffs.",
      },
    ],
  }),
